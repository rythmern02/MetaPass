...
contract NftStake is Ownable(msg.sender), IERC721Receiver {
    using EnumerableSet for EnumerableSet.UintSet;
    using EnumerableSet for EnumerableSet.AddressSet;
    using SafeERC20 for IERC20;

    // Constants
    uint256 public constant POINTS_PER_DAY = 10; // Points earned per day per NFT
    uint256 public constant MIN_STAKE_DURATION = 60 days; // Minimum duration required for staking
    address public constant DEFAULT_REWARD_TOKEN =
        0xda3828E745a2B1D74FBbd1Df202557A60Dd2F11b; // Default reward token

    // Configurable variables
    uint256 public maxNftsPerUser; // Maximum number of NFTs a user can stake
    ...
    function harvest(address _nftAddress) external {
        ...
        uint256 claimedRewards = user.claimedRewards;

        require(claimableRewards > claimedRewards, "No new rewards to claim");

        uint256 rewardToTransfer = claimableRewards - claimedRewards;

        require(
            IERC20(rewardToken).balanceOf(address(this)) >= rewardToTransfer,
            "Insufficient funds in treasury"
        );

        IERC20(rewardToken).safeTransfer(msg.sender, rewardToTransfer);

        // Update user state
        user.claimedRewards += rewardToTransfer;
        user.earnedPoints += pendingPoints;
        user.lastStakedTime = block.timestamp;

        emit Harvested(msg.sender, rewardToTransfer);
    }
    ...
    function claimPoints() external {
        ...
        for (uint256 i = 0; i < collectionAddresses.length(); i++) {
            ...
            if (user.stakedNfts[nftAddress].length() > 0) {
                hasStakedNFTs = true;
                break; // No need to continue checking if we've found staked NFTs
            }
        }
    ...
    function calculatePendingPoints(
        ...
        uint256 points = (stakedDuration * POINTS_PER_DAY * 1 ** 18);
        return points;
    }
    ...
    function calculateEarnedRewards(
        ...
        return points / 10; // 1 reward token per 10 points
    }
...
/// in last just need to correct the token for 10 points each day
